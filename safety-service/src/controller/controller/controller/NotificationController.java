...
public class NotificationController {
    ...
    @PostMapping("/send")
    public ResponseEntity<NotificationResponse> send(@RequestBody NotificationSendRequest request) {
        return ResponseEntity.ok(notificationService.send(request, null));
    }

    @GetMapping("/parent/{parentId}")
    public ResponseEntity<List<NotificationResponse>> getByParent(@PathVariable Long parentId) {
        return ResponseEntity.ok(notificationService.getByParent(parentId, null));
    }
}